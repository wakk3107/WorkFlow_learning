...
static inline int __set_fd_nonblock(int fd) {
  ...
  if (flags >= 0) flags = fcntl(fd, F_SETFL, flags | O_NONBLOCK);
...
int CommMessageIn::feedback(const void *buf, size_t size) {
  ...
  if (!entry->ssl) return write(entry->sockfd, buf, size);
  ...
  if (ret <= 0) {
    ...
    if (ret != SSL_ERROR_SYSCALL) errno = -ret;
...
void Communicator::release_conn(struct CommConnEntry *entry) {
  delete entry->conn;
  if (!entry->service) pthread_mutex_destroy(&entry->mutex);
...
int Communicator::send_message_sync(struct iovec vectors[], int cnt,
  ...
  while (cnt > 0) {
    if (!entry->ssl) {
      ...
      if (n < 0) return errno == EAGAIN ? cnt : -1;
...
int Communicator::send_message_async(struct iovec vectors[], int cnt,
  ...
  if (entry->write_iov) {
    for (i = 0; i < cnt; i++) entry->write_iov[i] = vectors[i];
  } else
    return -1;
  ...
  if (entry->state == CONN_STATE_IDLE) {
    ...
    if (ret < 0 && errno == ENOENT) entry->state = CONN_STATE_RECEIVING;
  } else {
    ret = mpoller_add(&data, timeout, this->mpoller);
    if (ret >= 0) {
      if (this->stop_flag) mpoller_del(data.fd, this->mpoller);
    }
  }

  if (ret < 0) {
    free(entry->write_iov);
    if (entry->state != CONN_STATE_RECEIVING) return -1;
  }
...
void Communicator::handle_incoming_request(struct poller_result *res) {
  ...
  if (entry) {
    if (session) session->handle(state, res->error);
...
void Communicator::handle_incoming_reply(struct poller_result *res) {
  ...
  if (entry) {
    ...
    if (__sync_sub_and_fetch(&entry->ref, 1) == 0) this->release_conn(entry);
  }
}
...
int Communicator::create_poller(size_t poller_threads) {
  ...
  if ((ssize_t)params.max_open_files < 0) return -1;
...
int Communicator::nonblock_listen(CommService *service) {
  ...
  if (sockfd >= 0) {
    if (__set_fd_nonblock(sockfd) >= 0) {
      if (__bind_and_listen(sockfd, service->bind_addr, service->addrlen) >=
          0) {
        return sockfd;
      }
    }
...
int Communicator::bind(CommService *service) {
  struct poller_data data;
  int sockfd;
  // 我替大家进去看了，就是初始化一下 listenfd，再 bind listen再返回该 fd
...
int Communicator::sleep(SleepSession *session) {
  ...
  if (session->duration(&value) >= 0) {
    if (mpoller_add_timer(&value, session, this->mpoller) >= 0) return 0;
  }
